...
const Timeline=()=>{
  ...
  const hideform = () => {
    console.log("HIDE")
    // axios.post('http:192.168.100.33:4444/imageup',data).then((response)=>{
    // // setrespo(response.data)
    // console.log(response)

    // })

    fetch("http:192.168.100.33:4444/imageup", {
      method: "POST",
...
useEffect(() => {
  axios.get('http:192.168.100.33:4444/getimage').then(response => {
    console.log(response.data);
    setPost(response.data);
  }).catch((error)=>{
    console.log('error occur in get call',error);
  })
...
<Text></Text><Text></Text>
        ...
        {post.map((item,index) => {
				...
				return ( 
        <View key={index} style={{marginTop:10 ,borderTopWidth:2,borderTopColor:'#f47b13' ,marginLeft:10,marginRight:10}}>
